...
##
## Validation of sexual network degree with longitudinal data
##
...
ind_orig <- read.csv("individuals.csv", stringsAsFactors = FALSE)
part <- read.csv("partners.csv", stringsAsFactors = FALSE)

# PARTNERS INCLUDE ONLY THOSE THAT MEET INCLUSION CRITERIA AND HAVE MALE PARTNERS
p <- filter(part, `X_INCLUDE` == 1 & GENDER == 1 & baseline_hiv == "Negative")
...
ind <- rename(ind, AGECAT = `X_AGECAT`)
...
## Marginal Tables for Degree (Supplemental Table 1)

table(ind_main$deg.dkn)
round(prop.table(table(ind_main$deg.dkn)), 3)

table(ind_casual$deg.dkn)
round(prop.table(table(ind_casual$deg.dkn)), 3)

table(ind_all$deg.dkn)
round(prop.table(table(ind_all$deg.dkn)), 3)

table(ind_main$deg.dkm)
round(prop.table(table(ind_main$deg.dkm)), 3)

table(ind_casual$deg.dkm)
round(prop.table(table(ind_casual$deg.dkm)), 3)

table(ind_all$deg.dkm)
round(prop.table(table(ind_all$deg.dkm)), 3)

table(ind_main$deg.adj)
round(prop.table(table(ind_main$deg.adj)), 3)

table(ind_casual$deg.adj)
round(prop.table(table(ind_casual$deg.adj)), 3)

table(ind_all$deg.adj)
round(prop.table(table(ind_all$deg.adj)), 3)


mean(ind_all$deg.adj - ind_all$deg.dkn)
mean(ind_all$deg.adj - ind_all$deg.dkm, na.rm = TRUE)
...
ggplot(preduN, aes(x = deg.dkn, y = (exp.est - deg.dkn))) +
  ...
  labs(x = "Degree at Baseline", y = "Bias in Confirmed Degree") +
  theme_minimal() +
  facet_grid( ~ type, scales = "free_x") +
  theme(strip.text.x = element_text(size = 12))

ggsave("Fig1.pdf", device = "pdf", height = 3.5, width = 7, units = "in")
ggsave("Fig1.jpg", device = "jpeg", height = 3.5, width = 7, units = "in")
...
pred.aaNc$RACE_INC <- ifelse(pred.aaNc$RACE_INC == "Black/African American", "Black", "White")
...
predN[predN$deg.dkn == 2, ]

ggplot(predN, aes(x = deg.dkn, y = exp.est)) +
  geom_point(aes(color = AGECATF)) +
  geom_line(aes(color = AGECATF)) +
  geom_abline(slope = 1, lty = 2) +
  geom_ribbon(data = predN, aes(ymin = lowCI, ymax = highCI, fill = AGECATF), alpha = 0.2) +
  labs(color = "Age Group", x = "Baseline Degree", y = "Predicted Confirmed Degree") +
  theme_minimal() +
...
ggsave("Fig2.pdf", device = "pdf", height = 5, width = 10, units = "in")
ggsave("Fig2.jpg", device = "jpeg", height = 5, width = 10, units = "in")
